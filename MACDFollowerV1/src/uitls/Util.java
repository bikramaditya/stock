...
public class Util 
{
	...
	public boolean isMarketOpen() {
		...
		if (now.isAfter(LocalDateTime.parse(today + "T09:00:00"))
				&& now.isBefore(LocalDateTime.parse(today + "T15:05:00")))
		{
			return true;
		}
		else
	...
	public ArrayList<MACDObj> getMACDFromAlpha(Stock stock, String interval, String AlphaApiKey) 
	{
		String DATE_FORMAT = "yyyy-MM-dd HH:mm";
		
		ArrayList<MACDObj> list = new ArrayList<MACDObj>();
		try {
			String myURL = "https://www.alphavantage.co/query?function=MACD&symbol="+stock.MKT+":"+stock.SYMBOL+"&interval="+interval+"&series_type=open&apikey="+AlphaApiKey;
			Document doc = Jsoup.connect(myURL).ignoreContentType(true).get();
			...
			JsonParser parser = new JsonParser();
			JsonObject o = parser.parse(JSONStr).getAsJsonObject();
			JsonObject macdObj = (JsonObject) o.get("Technical Analysis: MACD");
			
			
			for (Map.Entry<String, JsonElement> entry : macdObj.entrySet())  
			{
				String date_time = entry.getKey();
				LocalDateTime usdt = LocalDateTime.parse(date_time, DateTimeFormatter.ofPattern(DATE_FORMAT));
				
				LocalDateTime indt = usdt.plusMinutes(630);
				
				JsonObject macd_raw = (JsonObject)entry.getValue();
				
				MACDObj macd = new MACDObj();
				macd.date_time=indt.toString();
				
				JsonElement MACD_Hist = macd_raw.get("MACD_Hist");
				macd.Histogram = MACD_Hist.getAsDouble();
				
				JsonElement MACD = macd_raw.get("MACD");
				macd.MACD = MACD.getAsDouble();

				JsonElement MACD_Signal = macd_raw.get("MACD_Signal");
				macd.Signal = MACD_Signal.getAsDouble();
		...
		{
			Logger.log(1, "Could not get MACD "+e.getMessage());
			e.printStackTrace();
		}
		return list;
	}
